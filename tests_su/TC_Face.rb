...
class TC_Face < SUMT::TestCase

  TEMP_DIR = temp_dir

  def self.ste_teardown
    fn = File.join TEMP_DIR, "temp_image.jpg"
    File.delete(fn) if File.exist?(fn)
  end
  
  def setup
    @model = start_with_empty_model   # (clr_schemas: false)
  ...
  # moved to SUMT's utility library.

  def create_face(front = nil, back = nil)
  ...
  def create_face_with_glued_instance
    face = create_face
    component = @model.definitions.add("tester")
    point1 = [10, 10, 0]
    point2 = [20, 10, 0]
    point3 = [20, 20, 0]
    point4 = [10, 20, 0]
  ...
  def create_material
    full_name = File.join TEMP_DIR, "temp_image.jpg"
  ...
  def create_face_with_hole
    ary = [ [-1000, -1000, 0], [1000, -1000, 0], [1000, 1000,0], [-1000, 1000, 0] ]
    face = @model.active_entities.add_face(ary)
    create_face
    face
  end
  ...
  def test_all_connected_return_type
    assert_kind_of(Array, self.create_face.all_connected)
  end

  def test_all_connected_return_length
    assert_equal(5, self.create_face.all_connected.size)
  end

  def test_all_connected_return_faces_length
    connected = self.create_face.all_connected
  ...
  def test_all_connected_incorrect_number_of_arguments
    assert_raises(ArgumentError) { self.create_face.all_connected(1) }
  end
  ...
  def test_area_return_type
    assert_kind_of(Float, self.create_face.area)
  end

  def test_area_return_value
    assert_in_delta(10000.0, self.create_face.area, SKETCHUP_FLOAT_TOLERANCE)
  end
  ...
  def test_area_invalid_arguments
    face = create_face
    assert_raises(TypeError) { face.area("String!") }
    ...
    assert_raises(TypeError) { face.area false }
    assert_raises(TypeError) { face.area nil   }
  end

  def test_area_transformed
    transform = Geom::Transformation.new(
      [0.5, 0, 0, 0, 0, 0.5, 0, 0, 0, 0, 0.5, 0, 0, 0, 0, 1])
    area = self.create_face.area(transform)
  ...
  def test_area_valid_argument_array_transformation_matrix
    area = self.create_face.area([0.5, 0, 0, 0, 0, 0.5, 0, 0, 0, 0, 0.5, 0, 0, 0, 0, 1])
    assert_kind_of(Float, area)
  end
  ...
  def test_back_material_return_type
    face = create_face
    face.back_material = "red"
    assert_kind_of(Sketchup::Material, face.back_material)
  end
  ...
  def test_back_material_invalid_arguments
    face = create_face
    assert_raises(ArgumentError) { face.back_material "String!" }
    assert_raises(ArgumentError) { face.back_material ["Array"] }
    assert_raises(ArgumentError) { face.back_material false     }
    assert_raises(ArgumentError) { face.back_material nil       }
  end
  ...
  def test_back_material_Set_Integer
    face = create_face nil, 255
    assert_instance_of(Sketchup::Material, face.back_material )
  end

  def test_back_material_Set_HexInteger
    face = create_face nil, 0xff
    assert_instance_of(Sketchup::Material, face.back_material )
  end

  def test_back_material_Set_HexString
    face = create_face nil, '#ff0000'
    assert_instance_of(Sketchup::Material, face.back_material )
  end

  def test_back_material_Set_ArrayFloat
    face = create_face nil, [1.0, 0.0, 0.0]
    assert_instance_of(Sketchup::Material, face.back_material )
  end

  def test_back_material_Set_ArrayInteger
    face = create_face nil, [255, 0, 0]
    assert_instance_of(Sketchup::Material, face.back_material )
  end

  def test_back_material_Set_string
    face = create_face nil, "red"
    assert_instance_of(Sketchup::Material, face.back_material )
  end

  def test_back_material_Set_material_object
    material = @model.materials.add("Material")
    material.color = "red"
    face = create_face nil, material
    assert_instance_of(Sketchup::Material, face.back_material )
  end

  def test_back_material_Set_sketchupcolor
    face = create_face nil, Sketchup::Color.new("red")
    assert_instance_of(Sketchup::Material, face.back_material )
  end
  ...
  def test_back_material_Set_invalid_arguments
    skip("Fix this!") if SU_VERS_INT < 18
#    assert(false, "Fix this!") # Create orphan operation.
    skip("Broken in SU2014") if SU_VERS_INT == 14
    face = create_face
    assert_raises(ArgumentError) { face.back_material = "invalid color name" }
    assert_raises(ArgumentError) { face.back_material = ["Array"] }
    #assert_raises(TypeError) { face.back_material = nil }
  end
  ...
  def test_classify_point_outside
    ...
    assert_equal(Sketchup::Face::PointOutside, face.classify_point(pt) )
  end
  ...
  def test_classify_point_inside
    ...
    pt = Geom::Point3d.new(1, 1, 0)
    assert_equal(Sketchup::Face::PointInside, face.classify_point(pt) )
  end
  ...
  def test_classify_point_on_vertex
    ...
    pt = Geom::Point3d.new(0, 0, 0)
    result = 
    assert_equal(Sketchup::Face::PointOnVertex, face.classify_point(pt) )
  end
  ...
  def test_classify_point_on_edge
    ...
    assert_equal(Sketchup::Face::PointOnEdge, face.classify_point(pt) )
  end
  ...
  def test_classify_point_off_plane
    ...
    # Check a point that should be off the plane of the face.
    pt = Geom::Point3d.new(1, 1, 10)
    assert_equal(Sketchup::Face::PointNotOnPlane, face.classify_point(pt) )
  end
  ...
  def test_classify_point_invalid_arguments
    face = create_face
    assert_raises(ArgumentError) { face.classify_point "Strings are not valid!" }
    assert_raises(ArgumentError) { face.classify_point ["Array"]                }
    assert_raises(ArgumentError) { face.classify_point true }
    assert_raises(ArgumentError) { face.classify_point nil  }
  end
  ...
  def test_edges_return_type
    face = 
    assert_kind_of(Array, self.create_face.edges)
  end

  def test_edges_return_value
    edges = self.create_face.edges
    assert(edges.all? { |entity| entity.is_a?(Sketchup::Edge) },
      'Not all entities returned were edges.')
  end

  def test_edges_return_array_length
    assert_equal(4, self.create_face.edges.length)
  end
  ...
  def test_edges_invalid_arguments
    face = create_face
    assert_raises(ArgumentError) { face.edges "Strings are not valid!" }
    assert_raises(ArgumentError) { face.edges ["Can't use array"]      }
    assert_raises(ArgumentError) { face.edges 12    }
    assert_raises(ArgumentError) { face.edges false }
    assert_raises(ArgumentError) { face.edges nil   }
  end
  ...
  def test_followme_edge_argument_result
    ...
    point1 = Geom::Point3d.new(0, 0, 0)
    point2 = Geom::Point3d.new(0, 0, 100)
    edge = @model.active_entities.add_edges(point1, point2)
    assert_kind_of(TrueClass, face.followme(edge) )
  end
  ...
  def test_followme_edge_array_argument_result
    ...
    edge1 = @model.active_entities.add_edges(point1, point2)
    edge2 = @model.active_entities.add_edges(point2, point3)
    edge3 = @model.active_entities.add_edges(point3, point4)
  ...
  def test_followme_return_array_length
    face = create_face
    start_ents = @model.active_entities.length
    point1 = Geom::Point3d.new(0, 0, 0)
    point2 = Geom::Point3d.new(0, 0, 100)
    edge = @model.active_entities.add_edges(point1, point2)

    face.followme(edge)
    end_ents = @model.active_entities.length
  ...
  def test_followme_too_few_arguments
    face = create_face
    assert_raises(ArgumentError) { face.followme }
  end

  def test_followme_invalid_arguments
    face = create_face
    assert_raises(TypeError) { face.followme ["Can't use array"] }
    # assert_raises(ArgumentError) { face.followme nil }
  end
  ...
  def test_get_glued_instances_result_type
    glued_array = create_face_with_glued_instance.get_glued_instances
  ...
  def test_get_glued_instances_return_none_glued
    glued_array = self.create_face.get_glued_instances
  ...
  def test_get_glued_instances_invalid_arguments
    face = create_face
    assert_raises(ArgumentError) { face.get_glued_instances ["Can't use array"] }
    assert_raises(ArgumentError) { face.get_glued_instances 12    }
    assert_raises(ArgumentError) { face.get_glued_instances false }
    assert_raises(ArgumentError) { face.get_glued_instances nil   }
  end
  ...
  def test_get_texture_projection_return_type
    ...
    result = 
    assert_kind_of(Geom::Vector3d, face.get_texture_projection(true) )
  end
  ...
  def test_get_texture_projection_back_face_return_type
    ...
    assert_kind_of(Geom::Vector3d, face.get_texture_projection(false) )
  end
  ...
  def test_get_UVHelper_result_type_back_only
    tw = Sketchup.create_texture_writer
    uvHelp = self.create_face.get_UVHelper(false, true, tw)
    assert_kind_of(Sketchup::UVHelper, uvHelp)
  end

  def test_get_UVHelper_result_no_params
    uvHelp = self.create_face.get_UVHelper
    assert_kind_of(Sketchup::UVHelper, uvHelp)
  end
  ...
  def test_get_UVHelper_invalid_arguments
    face = create_face
    assert_raises(TypeError) { face.get_UVHelper [1], [1], [1]    }
    assert_raises(TypeError) { face.get_UVHelper nil, nil, nil    }
    assert_raises(TypeError) { face.get_UVHelper "st", "ri", "ng" }
  end
  ...
  def test_loops_return_type
    assert_kind_of(Array, create_face_with_hole.loops)
  end

  def test_loops_return_value
    loops = create_face_with_hole.loops
    assert(loops.all? { |entity| entity.is_a?(Sketchup::Loop) },
      'Not all entities returned were Loops.')
  end

  def test_loops_return_array_length
    face = create_face_with_hole
    assert_equal(2, face.loops.length)
  end
  ...
  def test_loops_invalid_arguments
    face = create_face_with_hole
    assert_raises(ArgumentError) { face.loops "Strings are not valid!" }
    assert_raises(ArgumentError) { face.loops ["Can't use array"]      }
    assert_raises(ArgumentError) { face.loops 12    }
    assert_raises(ArgumentError) { face.loops false }
    assert_raises(ArgumentError) { face.loops nil   }
  end
  ...
  def test_material
    face = create_face "red"
    assert_instance_of(Sketchup::Material, face.material)
  end
  ...
  def test_material_invalid_arguments
    face= create_face
    assert_raises(ArgumentError) { face.material "String!" }
    assert_raises(ArgumentError) { face.material ["Array"] }
    assert_raises(ArgumentError) { face.material false     }
    assert_raises(ArgumentError) { face.material nil       }
  end
  ...
  def test_material_Set_material_object
    ...
    material.color = "red"
    face = create_face material
    assert_instance_of(Sketchup::Material, face.material )
  end

  def test_material_Set_sketchupcolor
    face = create_face Sketchup::Color.new("red")
    assert_instance_of(Sketchup::Material, face.material )
  end
  ...
  def test_material_Set_deleted_material
    ...
    assert_raises(ArgumentError) { face.back_material = material }
  end
...
#    assert(false, "Fix this!") # Create orphan operation.
    skip("Broken in SU2014") if SU_VERS_INT == 14
    face = create_face
    assert_raises(ArgumentError) { face.material = "invalid color name" }
    assert_raises(ArgumentError) { face.material = ["Array"] }
    #assert_raises(TypeError) { face.material = nil }
  end
  ...
  def test_mesh_return_type
    ...
    assert_kind_of(Geom::PolygonMesh, face.mesh(7) )
  end
  ...
  def test_mesh_invalid_arguments
    face = create_face_with_hole
    assert_raises(TypeError) { face.mesh "Strings are not valid!" }
    assert_raises(TypeError) { face.mesh ["Can't use array"]      }
    assert_raises(TypeError) { face.mesh false                    }
    assert_raises(TypeError) { face.mesh nil                      }
  end

  def test_mesh_arguments_out_of_range
    face = create_face_with_hole
    # Should raise an error, but does not
    #assert_raises(TypeError) { face.mesh 12 }
    assert_raises(RangeError) { face.mesh -5 }
  end
  ...
  def test_normal_invalid_arguments
    face = create_face
    assert_raises(ArgumentError) { face.normal -5        }
    assert_raises(ArgumentError) { face.normal true      }
    assert_raises(ArgumentError) { face.normal "string"  }
    assert_raises(ArgumentError) { face.normal ["Array"] }
    assert_raises(ArgumentError) { face.normal nil       }
  end
  ...
  def test_outer_loop_return_type
    face = create_face_with_hole
    assert_kind_of(Sketchup::Loop, face.outer_loop)
  end
  ...
  def test_outer_loop_invalid_arguments
    face = create_face_with_hole
    assert_raises(ArgumentError) { face.outer_loop "Strings are not valid!" }
    assert_raises(ArgumentError) { face.outer_loop ["Can't use array"]      }
    assert_raises(ArgumentError) { face.outer_loop 12    }
    assert_raises(ArgumentError) { face.outer_loop false }
    assert_raises(ArgumentError) { face.outer_loop nil   }
  end
  ...
  def test_plane_return_type
    assert_kind_of(Array, self.create_face.plane)
  end

  def test_plane_return_value
    assert_equal([-0.0, -0.0, 1.0, -0.0], self.create_face.plane)
  end
  ...
  def test_plane_invalid_arguments
    face = create_face
    assert_raises(ArgumentError) { face.plane -5        }
    assert_raises(ArgumentError) { face.plane true      }
    assert_raises(ArgumentError) { face.plane "string"  }
    assert_raises(ArgumentError) { face.plane ["Array"] }
    assert_raises(ArgumentError) { face.plane nil       }
  end
  ...
  def test_position_material_invalid_arguments
    ...
    assert_raises(ArgumentError) { face.position_material "st", "ri", "ng"    }
    assert_raises(TypeError)     { face.position_material false, false, false }
    #assert_raises(ArgumentError) { face.position_material 1, 2, 3 }
    #assert_raises(ArgumentError) { face.position_material nil, nil, nil }
  end
  ...
  def test_pushpull_return_type
    assert_nil self.create_face.pushpull(100, true)
  end
  ...
  def test_pushpull_invalid_arguments
    ...
    assert_raises(TypeError) { face.pushpull "st", "ri"   }
    assert_raises(TypeError) { face.pushpull false, false }
    assert_raises(TypeError) { face.pushpull nil, nil     }
    #assert_raises(ArgumentError) { face.pushpull 1, 2  }
  end
  ...
  def test_reverse_Bang_invalid_arguments
    ...
    assert_raises(ArgumentError) { face.reverse! "st"  }
    assert_raises(ArgumentError) { face.reverse! false }
    assert_raises(ArgumentError) { face.reverse! nil   }
    assert_raises(ArgumentError) { face.reverse! 1, 2 }
  end
  ...
  def test_set_texture_projection_back_face_return_type
    face = create_face
    face.back_material = create_material
    vector = [rand, rand, rand]
  ...
  def test_set_texture_projection_invalid_arguments
    face = create_face
    assert_raises(ArgumentError) { face.set_texture_projection "st" , "ri"  }
    assert_raises(ArgumentError) { face.set_texture_projection false, false }
    assert_raises(ArgumentError) { face.set_texture_projection     1, 2     }
    #assert_raises(TypeError) { face.set_texture_projection nil, nil }
  end
  ...
  def test_vertices_invalid_arguments
    face = create_face
    assert_raises(ArgumentError) { face.vertices "Strings are not valid!" }
    assert_raises(ArgumentError) { face.vertices ["Can't use array"]      }
    assert_raises(ArgumentError) { face.vertices 12    }
    assert_raises(ArgumentError) { face.vertices false }
    assert_raises(ArgumentError) { face.vertices nil   }
  end
