...
class TC_Layers < SUMT::TestCase

  def setup
    model = start_with_empty_model()
    model.start_operation(self.name, true)
    5.times { |index|
      layer = model.layers.add("SUMT %02i" % index)
  ...
  def test_at_string
    layer = Sketchup.active_model.layers.at("SUMT 02")
    assert_kind_of(Sketchup::Layer, layer)
    assert_equal("SUMT 02", layer.name, "Incorrect layer name")
  end
  ...
  def test_at_incorrect_number_of_arguments_two
    layer = Sketchup.active_model.layers["SUMT 02"]
    assert_raises(ArgumentError) do
  ...
  def test_remove_name
    ...
    num_entities = Sketchup.active_model.entities.size
    num_layers = Sketchup.active_model.layers.size
    layer = layer = Sketchup.active_model.layers["SUMT 02"]
    Sketchup.active_model.layers.remove("SUMT 02")
    assert_equal(num_layers - 1, Sketchup.active_model.layers.size,
      "No layers erased")
    assert_equal(num_entities, Sketchup.active_model.entities.size,
      "Entities was erased")
    assert(layer.deleted?, "Layer not marked as deleted")
    assert_nil(Sketchup.active_model.layers["SUMT 02"])
  end
  ...
  def test_remove_name_erase_entities
    ...
    num_layers = Sketchup.active_model.layers.size
    layer = Sketchup.active_model.layers["SUMT 02"]
    Sketchup.active_model.layers.remove("SUMT 02", true)
    assert_equal(num_layers - 1, Sketchup.active_model.layers.size,
      "No layers erased")
    assert(Sketchup.active_model.entities.size < num_entities,
      "No entities erased")
  ...
  def test_remove_current
    ...
    current_layer = Sketchup.active_model.layers["SUMT 02"]
