...
module SUMT
 module Editor
  ...
  def self.application
    setting = SUMT.settings[:editor_application]
    if setting.nil?
      setting = SUMT.settings[:editor]
  ...
  def self.arguments
    setting = SUMT.settings[:editor_arguments]
    if setting.nil?
      setting = SUMT.settings[:editor]
      return nil unless setting.is_a?(String)
      # Legacy setting when it all was stored in a single string.
  ...
  def self.change(application, arguments)
    SUMT.settings[:editor] = nil
    SUMT.settings[:editor_application] = application
    SUMT.settings[:editor_arguments] = arguments
    nil
  end
